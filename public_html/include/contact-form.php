...
$subject = isset($_POST["widget-contact-form-subject"]) ? $_POST["widget-contact-form-subject"] : ' رسالة من موقع الشامر ';
...
if( $_SERVER['REQUEST_METHOD'] == 'POST') {
 ...
 if($email != '') {
                ...
                if ($_SERVER['HTTP_REFERER'] == 'http://alshamercpa.com/index.php')				
                $mail->Body = $name . $email . $phone . $company . $service . $message . '<br><br><br> طلب عرض سعر';
				else
				$mail->Body = $name . $email . $phone . $company . $service . $message . '<br><br><br>  رسالة إستفسار و تواصل ';
            ...
            if(isset($recaptcha) && $recaptcha == '') {
             $recaptcha_response=json_decode(file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=&response=".$recaptcha."&remoteip=".$_SERVER['REMOTE_ADDR']), true);
                        ...
                        {
                            $response = array ('response'=>'error', 'message'=> " الرجاء تأكيد هويتك البشرية ");
                        }
