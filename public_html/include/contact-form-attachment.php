...
if( $_SERVER['REQUEST_METHOD'] == 'POST') {
 ...
 if($email != '') {
                ...
                $name = isset($name) ? " الإسم  :  $name<br><br>" : '';
                $email = isset($email) ? " البريد الإلكتروني : $email<br><br>" : '';
                $phone = isset($phone) ? "Phone : $phone<br><br>" : '';
                $company = isset($company) ? "Company: $company<br><br>" : '';
                $service = isset($service) ? "Service: $service<br><br>" : '';
                $message = isset($message) ? " التفاصيل : $message<br><br>" : '';

                $mail->Body = $name . $email . $phone . $company . $service . $message . '<br><br><br>  طلب توظيف  ';
            ...
            if(isset($recaptcha) && $recaptcha == '') {
             $recaptcha_response=json_decode(file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=&response=".$recaptcha."&remoteip=".$_SERVER['REMOTE_ADDR']), true);
                        if($recaptcsha_response['success'] == false)
                        {
                            $response = array ('response'=>'error', 'message'=> " يرجى التحقق من حقل التأكد من هويتك البشرية ");
                        }
