...
class Mharty_Theme_Updater{
	...
	public function updater_data(){
		...
		/* Get the theme support arguements for 'mharty-theme-updater'. */
		$theme_support = get_theme_support( 'mharty-theme-updater' );
	...
	public function theme_source_selection( $source, $remote_source, $upgrader ){
		...
		if( isset( $upgrader->skin->theme_info->template ) ){
			...
			if ( $upgrader->skin->theme_info->template == get_template() ){
				...
				$upgrader->skin->feedback( esc_html__( 'Executing upgrader_source_selection filter function...', 'text-domain' ) );
				...
				if( isset( $source, $remote_source, $theme_name ) ){
					...
					if(@rename( $source, $new_source ) ){
						$upgrader->skin->feedback( esc_html__( 'Renamed theme folder successfully.', 'text-domain' ) );
						return $new_source;
					}
					/* unable to rename the folder to correct theme folder */
					else{
						$upgrader->skin->feedback( esc_html__( '**Unable to rename downloaded theme.', 'text-domain' ) );
						return new WP_Error();
					}
				}
				/* fallback */
				else
					$upgrader->skin->feedback( esc_html__( '**Source or Remote Source is unavailable.', 'text-domain' ) );
			}
		}
		return $source;
	}
	...
	public function add_dashboard_widget() {
		...
		/* Widget name */
		$widget_name = $updater_data['name'] . esc_html__( ' Theme Updates', 'text-domain' );
