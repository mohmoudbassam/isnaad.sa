<?php
class MhWidgetLatestPosts extends WP_Widget {

	function __construct() {
		$widget_ops = array('description' => esc_html__( 'Displays most recent posts with thumbnails.', 'mharty'), 'classname' => 'mh_list_posts');
		$control_ops = array( 'width' => 250, 'height' => 400 );
		parent::__construct( 'latest_posts', esc_html__( 'MH Latest Posts.', 'mharty' ), $widget_ops, $control_ops);
	...
	function widget( $args, $instance ) {
		...
		$cache = wp_cache_get( "Mh_Widget_Latest_Posts", "widget" );
		...
		$query = array( 'showposts' => $posts_number, 'nopaging' => 0, 'orderby' => 'date', 'order' => 'DSC', 'post_status' => 'publish', 'ignore_sticky_posts' => 1 );
   ...
   <?php the_post_thumbnail('mhc-post-thumbnail'); ?>
       ...
       <div class="clearfix"></div>
		...
		wp_cache_set( 'Mh_Widget_Latest_Posts', $cache, 'widget' );
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions['Mh_Widget_Latest_Posts'] ) )
			delete_option( 'Mh_Widget_Latest_Posts' );
	...
	function flush_widget_cache() {
		wp_cache_delete( 'Mh_Widget_Latest_Posts', 'widget' );
	}
...
}// end MhWidgetLatestPosts class

function MhWidgetLatestPostsInit() {
	register_widget( 'MhWidgetLatestPosts' );
}

add_action( 'widgets_init', 'MhWidgetLatestPostsInit' );