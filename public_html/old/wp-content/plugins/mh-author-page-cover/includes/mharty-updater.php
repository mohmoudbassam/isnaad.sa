...
if ( ! defined( 'ABSPATH' ) || class_exists( 'Mh_Author_Page_Cover_Updater', false ) )
	return;
 ...
 */
class Mh_Author_Page_Cover_Updater{
	...
	public function plugins_api_result( $res, $action, $args ) {
		...
		if ( $plugin_slug == $updater_data['slug'] && $action == 'plugin_information' ){
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', '<p>' . esc_html__( 'An Unexpected HTTP Error occurred during the API request.', 'text-domain' ) . '</p><p><a href="?" onclick="document.location.reload(); return false;">' . esc_html__( 'Try again', 'text-domain' ) . '</a></p>', $request->get_error_message() );
			}
			...
			/* If no error, construct the data */
				...
				/* If data is empty or not an object */
					...
					$res = new WP_Error( 'plugins_api_failed', esc_html__( 'An unknown error occurred', 'text-domain' ), wp_remote_retrieve_body( $request ) );
	...
	public function upgrader_post_install( $true, $hook_extra, $result ) {
		...
		if ( isset( $hook_extra ) ){
			...
			if ( isset( $hook_extra['plugin'] ) && $hook_extra['plugin'] == $plugin_base ){
				...
				/* Update message */
				$fail = esc_html__( 'The plugin has been updated, but could not be reactivated. Please reactivate it manually.', 'text-domain' );
				$success = esc_html__( 'Plugin reactivated successfully. ', 'text-domain' );
	...
	public function add_dashboard_widget() {
		...
		/* Widget name */
		$widget_name = $updater_data['name'] . esc_html__( ' Plugin Updates', 'text-domain' );
	...
	public function dashboard_widget_callback() {
		...
		if ( !empty( $widget_option ) && is_array( $widget_option ) ){
			...
			if ( true === $updater_data['role'] ){
				...
				echo '<p>'. esc_html__( 'Username: ', 'text-domain' ) . '<code>' . $username . '</code></p>';

				/* activation key input */
				$key = isset( $widget_option['key'] ) ? $widget_option['key'] : '' ;
				echo '<p>'. esc_html__( 'Email: ', 'text-domain' ) . '<code>' . $key . '</code></p>';
			}
			else{

				/* activation key input */
				$key = isset( $widget_option['key'] ) ? $widget_option['key'] : '' ;
				echo '<p>'. esc_html__( 'Key: ', 'text-domain' ) . '<code>' . $key . '</code></p>';
			}


			/* if key status is valid */
			if ( $widget_option['status'] == 'valid' ){
				esc_html_e( '<p>Your plugin update is <span style="color:green">active</span></p>', 'text-domain' );
			}
			/* if key is not valid */
			elseif( $widget_option['status'] == 'invalid' ){
				esc_html_e( '<p>Your input is <span style="color:red">not valid</span>, automatic updates is <span style="color:red">not active</span>.</p>', 'text-domain' );
				echo '<p><a href="' . $edit_url . '" class="button-primary">' . esc_html__( 'Edit Key', 'text-domain' ) . '</a></p>';
			}
			/* else */
			else{
				esc_html_e( '<p>Unable to validate update activation.</p>', 'text-domain' );
				echo '<p><a href="' . $edit_url . '" class="button-primary">' . esc_html__( 'Try again', 'text-domain' ) . '</a></p>';
			}
		}
		/* if activation key is not yet set/empty */
		else{
			echo '<p><a href="' . $edit_url . '" class="button-primary">' . esc_html__( 'Add Key', 'text-domain' ) . '</a></p>';
		}
	}
	...
	public function dashboard_widget_control_callback() {
		...
		<p>
			<label for="<?php echo $widget_id; ?>-username"><?php esc_html_e( 'User name', 'text-domain' ); ?></label>
		</p>
		<p>
			<input id="<?php echo $widget_id; ?>-username" name="<?php echo $widget_id; ?>[username]" type="text" value="<?php echo $username_option;?>"/>
		</p>
		<p>
			<label for="<?php echo $widget_id; ?>-key"><?php esc_html_e( 'Email', 'text-domain' ); ?></label>
		</p>
		<p>
		...
		<p>
			<label for="<?php echo $widget_id; ?>-key"><?php esc_html_e( 'Activation Key', 'text-domain' ); ?></label>
		</p>
		<p>
			<input id="<?php echo $widget_id; ?>-key" class="regular-text" name="<?php echo $widget_id; ?>[key]" type="text" value="<?php echo $key_option;?>"/>
		</p>
