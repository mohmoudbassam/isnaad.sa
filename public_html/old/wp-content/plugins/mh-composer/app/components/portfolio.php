<?php
class MHComposer_Component_Portfolio extends MHComposer_Component {
	function init() {
		$this->name = esc_html__( 'Portfolio', 'mh-composer' );
		$this->slug = 'mhc_portfolio';
		...
		$this->custom_css_options = array(
			'portfolio_item' => array(
				'label'    => esc_html__( 'Portfolio Item', 'mh-composer' ),
				'selector' => '.mhc_portfolio_item',
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		if ( have_posts() ) {
			...
			?>
				<div class="meta-container <?php if ('on' === $show_loveit ){ echo 'with-loveit';}?>">
				...
				<?php if (( 'on' === $show_loveit ) && ( function_exists('mh_loveit') )): mh_loveit();  endif; ?>
				</div> <!-- .mhc_portfolio_item -->
	<?php	}

			if ( 'on' === $show_pagination && ! is_search() ) {
				echo '</div> <!-- .mhc_portfolio -->';
		...
		$output = sprintf(
			'<div%5$s class="%1$s%3$s%6$s%7$s">
				%2$s
			%4$s',
			( 'on' === $fullwidth ? 'mhc_portfolio' : 'mhc_portfolio_grid clearfix' ),
			$posts,
			esc_attr( $class ),
			( ! $container_is_closed ? '</div> <!-- .mhc_portfolio -->' : '' ),
...
new MHComposer_Component_Portfolio;
