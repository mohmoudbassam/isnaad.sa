<?php
class MHComposer_Component_Poster extends MHComposer_Component {
	function init() {
		$this->name            = esc_html__( 'Poster', 'mh-composer' );
		$this->slug            = 'mhc_texton';
		...
		$this->fields_defaults = array(
			...
			'use_border'			  => array( 'off' ),
			'border_color'			=> array( $accent_color, 'append_default' ),
			'border_style'			=> array( 'solid' ),
			'border_hover'			=> array( 'off' ),
		);
	...
	function get_fields() {
		...
		$fields = array(
			...
			'use_border' => array(
				'label'             => esc_html__( 'Use Border', 'mh-composer' ),
				'type'              => 'switch_button',
				'options'           => array(
					'off'     => esc_html__( 'No', 'mh-composer' ),
					'on'      => esc_html__( 'Yes', 'mh-composer' ),
			...
			'border_width'  => array(
				'label'             => esc_html__( 'Border Thickness', 'mh-composer' ),
				'type'              => 'range',
				'depends_show_if'   => 'on',
				'default' => '5',
				'tab_slug'        => 'advanced',
			...
			'border_style' => array(
				'label'             => esc_html__( 'Border Style', 'mh-composer' ),
				'type'              => 'select',
			...
			'border_hover' => array(
				'label'           => esc_html__( 'Show Border on Hover', 'mh-composer' ),
				'type'            => 'switch_button',
				'options'         => array(
					'off' => esc_html__( 'No', 'mh-composer' ),
					'on'  => esc_html__( 'Yes', 'mh-composer' ),
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		$use_border		  = $this->shortcode_atts['use_border'];
		...
		$border_hover		= $this->shortcode_atts['border_hover'];
		...
		if ( 'on' === $use_border ) {
			...
			MHComposer_Core::set_style( $function_name, array(
				'selector'    => '%%order_class%% .mhc_texton_border',
				'declaration' => sprintf( ' border-color:%1$s; border-style:%2$s;',
				esc_attr( $border_color ),
				( 'solid' !== $border_style ? esc_attr( $border_style ) : 'solid')
				)
			) );
			
			$style .= sprintf( ' border-width: %1$spx;',
				( '5' !== $border_width ? esc_attr( $border_width ) : '5')
			);
			
			MHComposer_Core::set_style( $function_name, array(
				'selector'    => sprintf('%%order_class%%.mhc_texton%1$s .mhc_texton_border',
					'on' === $border_hover ? ':hover' : ''
				),
				'declaration' => sprintf( ' border-width: %1$spx;',
					( '5' !== $border_width ? esc_attr( $border_width ) : '5')
				)
			) );
		}
		...
		if ( 'on' === $show_in_lightbox ) {
			$url_output = sprintf( 'href="%1$s" class="mhc_lightbox_image" title="%2$s"',
				esc_url( $src ),
				esc_attr( $alt )
			);
		} else if ( '' !== $url ) {
	...
	$output = sprintf(
		'<div%1$s class="mh-waypoint mhc_texton%2$s%3$s%4$s%12$s">
		%5$s
		%7$s
		%11$s
		...
		'on' === $use_border ? '<div class="mhc_texton_border"></div>' : '',
		('on' === $use_border ? sprintf('%1$s',
		'on' === $border_hover ? ' mhc_textone_has_border_hover'  : ' mhc_textone_has_border'
		) : '')
	);
...
new MHComposer_Component_Poster;