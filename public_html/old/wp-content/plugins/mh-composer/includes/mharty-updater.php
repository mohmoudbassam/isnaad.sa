...
if ( ! defined( 'ABSPATH' ) || class_exists( 'Mh_Composer_Updater', false ) )
	return;
...
class Mh_Composer_Updater{
	...
	public function plugins_api_result( $res, $action, $args ) {
		...
		if ( $plugin_slug == $updater_data['slug'] && $action == 'plugin_information' ){
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', '<p>' . esc_html__( 'An Unexpected HTTP Error occurred during the API request.', 'text-domain' ) . '</p><p><a href="?" onclick="document.location.reload(); return false;">' . esc_html__( 'Please try again.', 'text-domain' ) . '</a></p>', $request->get_error_message() );
			}
	...
	public function dashboard_widget_callback() {
		...
		if ( !empty( $widget_option ) && is_array( $widget_option ) ){
			...
			/* else */
				...
				echo '<p><a href="' . $edit_url . '" class="button-primary">' . esc_html__( 'Please try again.', 'text-domain' ) . '</a></p>';
			}
		}
	...
	public function dashboard_widget_control_callback() {
		...
		<p>
			<label for="<?php echo $widget_id; ?>-username"><?php esc_html_e( 'Username', 'text-domain' ); ?></label>
		</p>
		<p>
