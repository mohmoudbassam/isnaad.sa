<?php
/**
 * Handles the impoexpo workflow.
 * @package App\Import & Export (impoexpo)
 */
final class MH_App_ImpoExpo {
	...
	public function __construct( $context ) {
		...
		if ( ! $this->instance = mh_app_cache_get( $context, 'mh_app_impoexpo' ) ) {
			return false;
		}
	...
	public function import() {
		// Verify nonce.
		if ( ! ( isset( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], 'mh_app_impoexpo_nonce' ) ) ) {
			wp_send_json_error();
		}
		...
		$temp_file = $this->has_temp_file( $temp_file_id, 'mh_app_import' );
		...
		if ( $temp_file ) {
			...
			$temp_file = $this->temp_file( $temp_file_id, 'mh_app_import', $upload['file'] );
			$import = json_decode( $filesystem->get_contents( $temp_file ), true );
		...
		$this->delete_temp_files( 'mh_app_import' );
	...
	public function export() {
		...
		$filesystem = $this->set_filesystem();
		$temp_file_id = sanitize_file_name( $timestamp );
		$temp_file = $this->has_temp_file( $temp_file_id, 'mh_app_export' );

		if ( $temp_file ) {
			$data = json_decode( $filesystem->get_contents( $temp_file ), true );
		} else {
			$temp_file = $this->temp_file( $temp_file_id, 'mh_app_export' );
	...
	public function download_export() {
		if ( ! isset( $_GET['nonce'] ) && wp_verify_nonce( $_GET['nonce'], 'mh_app_impoexpo_nonce' ) ) {
			wp_die( esc_html__( 'The export process failed. Please refresh the page and try again.', MHARTY_APP_TEXTDOMAIN ) );
		}
		...
		$filesystem = $this->set_filesystem();
		$temp_file = $this->temp_file( sanitize_file_name( $timestamp ), 'mh_app_export' );
		...
		$this->delete_temp_files( 'mh_app_export' );
	...
	private function export_posts_query() {
		...
		$get_posts = get_posts( apply_filters( "mh_app_impoexpo_export_wp_query_{$this->instance->context}", $args ) );
	...
	/**
	 * Import post.
	 * @param array $posts Array of data formated by the impoexpo exporter.
	 */
	...
	private function maybe_paginate_images( $images, $method, $timestamp ) {
		if ( count( $images ) > 5 ) {
			...
			$this->delete_temp_files( 'mh_app_export', array( $temp_file_id => $temp_file ) );
		} else {
			$images = $this->$method( $images );
		}
	...
	private function has_temp_file( $id, $group ) {
		$temp_files = get_option( '_mh_app_impoexpo_temp_files', array() );
	...
	private function temp_file( $id, $group, $temp_file = false ) {
		...
		update_option( '_mh_app_impoexpo_temp_files', $temp_files, false );
	...
	public function delete_temp_files( $group = false, $defined_files = false ) {
		...
		if ( empty( $temp_files ) ) {
			delete_option( '_mh_app_impoexpo_temp_files' );
		} else {
			update_option( '_mh_app_impoexpo_temp_files', $temp_files, false );
		}
	}
	...
	public function assets() {
		$time = '<span>1</span>';
		$ltr = is_rtl() ? "" : "-ltr";
		wp_enqueue_style( 'mh-app-impoexpo', MHARTY_APP_URL . 'admin/css/impoexpo' . $ltr . '.css', array(
			'mh-app-admin',
		), MHARTY_APP_VERSION );
		wp_enqueue_script( 'mh-app-impoexpo', MHARTY_APP_URL . 'admin/js/impoexpo.js', array(
			'jquery',
			'jquery-ui-tabs',
			'jquery-form',
			'mh-app-admin',
		), MHARTY_APP_VERSION );
		wp_localize_script( 'mh-app-impoexpo', 'mhAppImpoExpo', array(
			'nonce'         => wp_create_nonce( 'mh_app_impoexpo_nonce' ),
			'postMaxSize'   => (int) @ini_get( 'post_max_size' ),
			'uploadMaxSize' => (int) @ini_get( 'upload_max_filesize' ),
			'text'          => array(
				'browserSupport'      => esc_html__( 'The browser version you are currently using is outdated. Please update to the latest version.', MHARTY_APP_TEXTDOMAIN ),
				'memoryExhausted'     => esc_html__( 'You reached your server memory limit. Please try increasing your PHP memory limit.', MHARTY_APP_TEXTDOMAIN ),
				'maxSizeExceeded'     => esc_html__( 'This file cannot be imported. It may be caused by file_uploads being disabled in your php.ini. It may also be caused by post_max_size or/and upload_max_filesize being smaller than file selected. Please contact your website hosting company.', MHARTY_APP_TEXTDOMAIN ),
				'invalideFile'        => esc_html__( 'Invalid File format. You should be uploading a JSON file.', MHARTY_APP_TEXTDOMAIN ),
				'importContextFail'   => esc_html__( 'This file should not be imported in this context.', MHARTY_APP_TEXTDOMAIN ),
				'importing'           => sprintf( esc_html__( 'Import estimated time remaining: %smin', MHARTY_APP_TEXTDOMAIN ), $time ),
				'exporting'           => sprintf( esc_html__( 'Export estimated time remaining: %smin', MHARTY_APP_TEXTDOMAIN ), $time ),
				'backuping'           => sprintf( esc_html__( 'Backup estimated time remaining: %smin', MHARTY_APP_TEXTDOMAIN ), $time ),
			),
		) );
	}
	...
	public function modal() {
		...
		?>
		<div class="mh-app-modal-overlay mh-app-form" data-mh-app-impoexpo="<?php echo $this->instance->context; ?>">
			<div class="mh-app-modal">
				<div class="mh-app-modal-header">
					<h3 class="mh-app-modal-title"><?php esc_html_e( 'Import/Export', MHARTY_APP_TEXTDOMAIN ); ?></h3><a href="#" class="mh-app-modal-close" data-mh-app-modal="close"></a>
				</div>
				<div data-mh-app-tabs>
					<ul class="mh-app-tabs">
						<li><a href="#mh-app-impoexpo-import"><?php esc_html_e( 'Import', MHARTY_APP_TEXTDOMAIN ); ?></a></li>
                        <li><a href="#mh-app-impoexpo-export"><?php esc_html_e( 'Export', MHARTY_APP_TEXTDOMAIN ); ?></a></li>
					</ul>
					<div id="mh-app-impoexpo-import">
						<div class="mh-app-modal-content">
							<?php if ( 'post' === $this->instance->type ) : ?>
								<?php printf( esc_html__( 'Importing %s file will overwrite all content currently on this page.', MHARTY_APP_TEXTDOMAIN ), $this->instance->name ); ?>
							<?php elseif ( 'post_type' === $this->instance->type ) : ?>
								<h3><?php printf( esc_html__( 'Select Mharty - Page Composer Layouts file to begin the import process:', MHARTY_APP_TEXTDOMAIN ), $this->instance->name ); ?></h3>
							<?php else : ?>
								<?php printf( esc_html__( 'Importing %s file will overwrite all current data. Please proceed with caution!', MHARTY_APP_TEXTDOMAIN ), $this->instance->name ); ?>
							<?php endif; ?>
							<?php /*?><h3><?php esc_html_e( 'Select File To Import', MHARTY_APP_TEXTDOMAIN ); ?></h3><?php */?>
							<form class="mh-app-impoexpo-import-form">
								<span class="mh-app-impoexpo-import-placeholder"><?php esc_html_e( 'No File Selected', MHARTY_APP_TEXTDOMAIN ); ?></span>
								<button class="mh-app-button"><?php esc_html_e( 'Choose File', MHARTY_APP_TEXTDOMAIN ); ?></button>
								<input type="file">
								<?php if ( 'post_type' !== $this->instance->type ) : ?>
									<div class="mh-app-clearfix"></div>
									<label><input type="checkbox" name="mh-app-impoexpo-import-backup" /><?php esc_html_e( 'Download backup before importing', MHARTY_APP_TEXTDOMAIN ); ?></label>
								<?php endif; ?>
							</form>
                            <?php if ( 'post_type' === $this->instance->type ) : ?>
                                    <small><?php printf( esc_html__( 'Note: Large files or files contain large images may take several minutes to upload.', MHARTY_APP_TEXTDOMAIN ), $this->instance->name ); ?></small>
                                <?php endif; ?>
						</div>
						<a class="mh-app-modal-action mh-app-impoexpo-import" href="#"><?php printf( esc_html__( 'Import %s', MHARTY_APP_TEXTDOMAIN ), $this->instance->name ); ?></a>
						<a class="mh-app-modal-action mh-app-button-danger" href="#" data-mh-app-impoexpo-cancel><?php esc_html_e( 'Cancel Import', MHARTY_APP_TEXTDOMAIN ); ?></a>
					</div>
                    <div id="mh-app-impoexpo-export">
						<div class="mh-app-modal-content">
							<?php printf( esc_html__( 'Exporting your %s will create a JSON file that can be imported later, or into a different website uses Mharty Theme.', MHARTY_APP_TEXTDOMAIN ), $this->instance->name ); ?>
							<h3><?php esc_html_e( 'Export File Name', MHARTY_APP_TEXTDOMAIN ); ?></h3>
							<form class="mh-app-impoexpo-export-form">
								<input type="text" name="" value="<?php echo esc_attr( $this->instance->name ); ?>">
								<?php if ( 'post_type' === $this->instance->type ) : ?>
									<div class="mh-app-clearfix"></div>
									<label><input type="checkbox" name="mh-app-impoexpo-posts"/><?php esc_html_e( 'Only export selected items', MHARTY_APP_TEXTDOMAIN ); ?></label>
								<?php endif; ?>
							</form>
						</div>
						<a class="mh-app-modal-action" href="#" data-mh-app-impoexpo-export="<?php echo esc_url( $export_url ); ?>"><?php printf( esc_html__( 'Export %s', MHARTY_APP_TEXTDOMAIN ), $this->instance->name ); ?></a>
						<a class="mh-app-modal-action mh-app-button-danger" href="#" data-mh-app-impoexpo-cancel><?php esc_html_e( 'Cancel Export', MHARTY_APP_TEXTDOMAIN ); ?></a>
					</div>
				</div>
			</div>
		</div>
